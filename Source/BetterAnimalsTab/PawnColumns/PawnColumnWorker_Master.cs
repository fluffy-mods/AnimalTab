...
namespace AnimalTab {
    public class PawnColumnWorker_Master: RimWorld.PawnColumnWorker_Master {
        ...
        public static FloatMenuOption MassAssignMaster_FloatMenuOption(Pawn colonist, PawnTable table) {
            IEnumerable<Pawn> animals = ObedientAnimals( table );
            if (colonist == null) {
            ...
            int skill = colonist.skills.GetSkill( SkillDefOf.Animals ).Level;
            IEnumerable<Pawn> eligibleAnimals =
        ...
        public static void MassAssignMasterBonded(PawnTable table) {
            ...
            foreach (Pawn animal in ObedientAnimals(table)) {
                // get bond
                Pawn bond = animal.relations.GetFirstDirectRelationPawn( PawnRelationDefOf.Bond,
